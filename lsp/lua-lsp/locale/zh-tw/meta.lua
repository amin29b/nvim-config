...
collectgarbage51    =
[[
此函式是垃圾回收器的通用介面。根據其第一個引數 opt 執行不同的功能：

* `"collect"`：執行一個完整的垃圾回收週期。這是預設選項。
* `"stop"`：停止垃圾回收器。
* `"restart"`：重新啟動垃圾回收器。
* `"count"`：回傳 Lua 使用的總記憶體數（以 K 位元組為單位）。
* `"step"`：執行一次垃圾回收步驟。步驟的「大小」由 arg 控制（較大的值表示更多步驟），具體方式未指定。如果要控制步驟大小，必須透過實驗調整 arg 的值。如果步驟完成了一個回收週期則回傳 true。
* `"setpause"`：將 arg 設定為回收器間歇率的新值（參見 §2.10）。回傳間歇率的先前值。
* `"setstepmul"`：將 arg 設定為回收器步進倍率的新值（參見 §2.10）。回傳步進倍率的先前值。
]]
...
* `"collect"`：執行一個完整的垃圾回收週期。這是預設選項。
* `"stop"`：停止垃圾回收器的自動執行。回收器將僅在顯式呼叫時執行，直到呼叫重新啟動它。
* `"restart"`：重新啟動垃圾回收器的自動執行。
* `"count"`：回傳 Lua 使用的總記憶體數（以 K 位元組為單位）和第二個值，表示總記憶體位元組數模 1024。第一個值有小數部分，因此以下等式始終為真：
    ```lua
     k, b = collectgarbage("count")
     assert(k*1024 == math.floor(k)*1024 + b)
     ```
    （當 Lua 使用非浮點型別編譯數字時，第二個結果很有用。）
* `"step"`：執行一次垃圾回收步驟。步驟的「大小」由 arg 控制（較大的值表示更多步驟），具體方式未指定。如果要控制步驟大小，必須透過實驗調整 arg 的值。如果步驟完成了一個回收週期則回傳 true。
* `"setpause"`：將 arg 設定為回收器間歇率的新值（參見 §2.5）。回傳間歇率的先前值。
* `"setstepmul"`：將 arg 設定為回收器步進倍率的新值（參見 §2.5）。回傳步進倍率的先前值。
* `"isrunning"`：回傳一個布林值，指示回收器是否正在執行（即未停止）。
* `"generational"`：將回收器變更為分代模式。這是一個實驗性功能（參見 §2.5）。
* `"incremental"`：將回收器變更為增量模式。這是預設模式。
]]
...
* `"stop"`：停止垃圾回收器的自動執行。回收器將僅在顯式呼叫時執行，直到呼叫重新啟動它。
* `"restart"`：重新啟動垃圾回收器的自動執行。
* `"count"`：回傳 Lua 使用的總記憶體數（以 K 位元組為單位）。該值有小數部分，因此乘以 1024 即可得到 Lua 使用的精確位元組數（除溢位外）。
* `"step"`：執行一次垃圾回收步驟。步驟的「大小」由 arg 控制。值為零時，回收器將執行一個基本（不可分割的）步驟。對於非零值，回收器的行為就像 Lua 分配了相應數量的記憶體（以 K 位元組為單位）。如果步驟完成了一個回收週期則回傳 true。
* `"setpause"`：將 arg 設定為回收器間歇率的新值（參見 §2.5）。回傳間歇率的先前值。
* `"setstepmul"`：將 arg 設定為回收器步進倍率的新值（參見 §2.5）。回傳步進倍率的先前值。
* "isrunning"`：回傳一個布林值，指示回收器是否正在執行（即未停止）。
]]
...
* `"stop"`：停止垃圾回收器的自動執行。回收器將僅在顯式呼叫時執行，直到呼叫重新啟動它。
* `"restart"`：重新啟動垃圾回收器的自動執行。
* `"count"`：回傳 Lua 使用的總記憶體數（以 K 位元組為單位）。該值有小數部分，因此乘以 1024 即可得到 Lua 使用的精確位元組數。
* `"step"`：執行一次垃圾回收步驟。步驟的「大小」由 arg 控制。值為零時，回收器將執行一個基本（不可分割的）步驟。對於非零值，回收器的行為就像 Lua 分配了相應數量的記憶體（以 K 位元組為單位）。如果步驟完成了一個回收週期則回傳 true。
* `"isrunning"`：回傳一個布林值，指示回收器是否正在執行（即未停止）。
* `"incremental"`：將回收器模式變更為增量模式。此選項後可以接三個數字：垃圾回收器間歇率、步進倍率和步進大小（參見 §2.5.1）。零表示不變更該值。
* `"generational"`：將回收器模式變更為分代模式。此選項後可以接兩個數字：垃圾回收器次要乘數和主要乘數（參見 §2.5.2）。零表示不變更該值。
有關垃圾回收和其中一些選項的更多詳細資訊，請參見 §2.5。
...
* `"restart"`：重新啟動垃圾回收器的自動執行。
* `"count"`：回傳 Lua 使用的總記憶體數（以 K 位元組為單位）。該值有小數部分，因此乘以 1024 即可得到 Lua 使用的精確位元組數。
* `"step"`：執行一次垃圾回收步驟。此選項後可以接一個額外引數，一個表示步進大小的整數。

    如果大小為正數 n，回收器的行為就像分配了 n 個新位元組。如果大小為零，回收器執行一個基本步驟。在增量模式下，基本步驟對應於目前步進大小。在分代模式下，基本步驟執行一次完整的次要回收，或者如果回收器已安排了增量步驟，則執行增量步驟。

    在增量模式下，如果步驟完成了一個回收週期，函式回傳 true。在分代模式下，如果步驟完成了一次主要回收，函式回傳 true。

* `"isrunning"`：回傳一個布林值，指示回收器是否正在執行（即未停止）。
* `"incremental"`：將回收器模式變更為增量模式並回傳先前的模式。
* `"generational"`：將回收器模式變更為分代模式並回傳先前的模式。
* `"param"`：變更和/或檢索回收器參數的值。此選項後必須接一個或兩個額外引數：被變更或檢索的參數名稱（字串）和該參數的可選新值，範圍為 [0,100000] 的整數。第一個引數必須是以下值之一：
    * `"minormul"`：次要乘數。
    * `"majorminor"`：主要-次要乘數。
    * `"minormajor"`：次要-主要乘數。
    * `"pause"`：垃圾回收器間歇率。
    ...
    * `"stepsize"`：步進大小。
    ...
    呼叫總是回傳參數的先前值。如果呼叫未提供新值，則該值保持不變。

    Lua 以壓縮格式儲存這些值，因此作為先前值回傳的值可能不完全等於上次設定的值。
