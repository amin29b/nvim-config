...
collectgarbage51    =
[[
この関数はガベージコレクタへの汎用インターフェースです。最初の引数 opt に応じて、さまざまな機能を実行します：

* `"collect"`：完全なガベージコレクションサイクルを実行します。これがデフォルトオプションです。
* `"stop"`：ガベージコレクタを停止します。
* `"restart"`：ガベージコレクタを再起動します。
* `"count"`：Lua が使用しているメモリの合計（キロバイト単位）を返します。
* `"step"`：ガベージコレクションステップを実行します。ステップの「サイズ」は arg によって制御されます（より大きな値はより多くのステップを意味します）が、具体的な方法は規定されていません。ステップサイズを制御したい場合は、arg の値を実験的に調整する必要があります。ステップが収集サイクルを完了した場合、true を返します。
* `"setpause"`：arg をコレクタの一時停止の新しい値として設定します（§2.10 参照）。一時停止の以前の値を返します。
* `"setstepmul"`：arg をコレクタのステップ乗数の新しい値として設定します（§2.10 参照）。ステップ乗数の以前の値を返します。
]]
...
* `"collect"`：完全なガベージコレクションサイクルを実行します。これがデフォルトオプションです。
* `"stop"`：ガベージコレクタの自動実行を停止します。コレクタは明示的に呼び出された場合にのみ実行され、再起動の呼び出しまで続きます。
* `"restart"`：ガベージコレクタの自動実行を再開します。
* `"count"`：Lua が使用しているメモリの合計（キロバイト単位）と、メモリの総バイト数を 1024 で割った余りを示す 2 番目の値を返します。最初の値には小数部分があるため、次の等式は常に真です：
    ```lua
     k, b = collectgarbage("count")
     assert(k*1024 == math.floor(k)*1024 + b)
     ```
    （Lua が数値に非浮動小数点型でコンパイルされている場合、2 番目の結果が役立ちます。）
* `"step"`：ガベージコレクションステップを実行します。ステップの「サイズ」は arg によって制御されます（より大きな値はより多くのステップを意味します）が、具体的な方法は規定されていません。ステップサイズを制御したい場合は、arg の値を実験的に調整する必要があります。ステップが収集サイクルを完了した場合、true を返します。
* `"setpause"`：arg をコレクタの一時停止の新しい値として設定します（§2.5 参照）。一時停止の以前の値を返します。
* `"setstepmul"`：arg をコレクタのステップ乗数の新しい値として設定します（§2.5 参照）。ステップ乗数の以前の値を返します。
* `"isrunning"`：コレクタが実行中かどうか（つまり、停止されていないか）を示すブール値を返します。
* `"generational"`：コレクタを世代別モードに変更します。これは実験的な機能です（§2.5 参照）。
* `"incremental"`：コレクタをインクリメンタルモードに変更します。これがデフォルトモードです。
]]
...
* `"stop"`：ガベージコレクタの自動実行を停止します。コレクタは明示的に呼び出された場合にのみ実行され、再起動の呼び出しまで続きます。
* `"restart"`：ガベージコレクタの自動実行を再開します。
* `"count"`：Lua が使用しているメモリの合計をキロバイト単位で返します。値には小数部分があり、1024 を掛けると Lua が使用している正確なバイト数が得られます（オーバーフローを除く）。
* `"step"`：ガベージコレクションステップを実行します。ステップの「サイズ」は arg によって制御されます。値がゼロの場合、コレクタは 1 つの基本（不可分な）ステップを実行します。ゼロ以外の値の場合、コレクタはその量のメモリ（キロバイト単位）が Lua によって割り当てられたかのように動作します。ステップが収集サイクルを完了した場合、true を返します。
* `"setpause"`：arg をコレクタの一時停止の新しい値として設定します（§2.5 参照）。一時停止の以前の値を返します。
* `"setstepmul"`：arg をコレクタのステップ乗数の新しい値として設定します（§2.5 参照）。ステップ乗数の以前の値を返します。
* "isrunning"`：コレクタが実行中かどうか（つまり、停止されていないか）を示すブール値を返します。
]]
...
* `"stop"`：ガベージコレクタの自動実行を停止します。コレクタは明示的に呼び出された場合にのみ実行され、再起動の呼び出しまで続きます。
* `"restart"`：ガベージコレクタの自動実行を再開します。
* `"count"`：Lua が使用しているメモリの合計をキロバイト単位で返します。値には小数部分があり、1024 を掛けると Lua が使用している正確なバイト数が得られます。
* `"step"`：ガベージコレクションステップを実行します。ステップの「サイズ」は arg によって制御されます。値がゼロの場合、コレクタは 1 つの基本（不可分な）ステップを実行します。ゼロ以外の値の場合、コレクタはその量のメモリ（キロバイト単位）が Lua によって割り当てられたかのように動作します。ステップが収集サイクルを完了した場合、true を返します。
* `"isrunning"`：コレクタが実行中かどうか（つまり、停止されていないか）を示すブール値を返します。
* `"incremental"`：コレクタモードをインクリメンタルに変更します。このオプションの後には 3 つの数値を続けることができます：ガベージコレクタの一時停止、ステップ乗数、およびステップサイズ（§2.5.1 参照）。ゼロはその値を変更しないことを意味します。
* `"generational"`：コレクタモードを世代別に変更します。このオプションの後には 2 つの数値を続けることができます：ガベージコレクタのマイナー乗数とメジャー乗数（§2.5.2 参照）。ゼロはその値を変更しないことを意味します。
ガベージコレクションとこれらのオプションの一部の詳細については、§2.5 を参照してください。
...
* `"restart"`：ガベージコレクタの自動実行を再開します。
* `"count"`：Lua が使用しているメモリの合計をキロバイト単位で返します。値には小数部分があり、1024 を掛けると Lua が使用している正確なバイト数が得られます。
* `"step"`：ガベージコレクションステップを実行します。このオプションの後には、ステップサイズを示す整数の追加引数を続けることができます。

    サイズが正の n の場合、コレクタは n 個の新しいバイトが割り当てられたかのように動作します。サイズがゼロの場合、コレクタは基本ステップを実行します。インクリメンタルモードでは、基本ステップは現在のステップサイズに対応します。世代別モードでは、基本ステップは完全なマイナーコレクション、またはコレクタがスケジュールしている場合はインクリメンタルステップを実行します。

    インクリメンタルモードでは、ステップが収集サイクルを完了した場合、関数は true を返します。世代別モードでは、ステップがメジャーコレクションを完了した場合、関数は true を返します。

* `"isrunning"`：コレクタが実行中かどうか（つまり、停止されていないか）を示すブール値を返します。
* `"incremental"`：コレクタモードをインクリメンタルに変更し、以前のモードを返します。
* `"generational"`：コレクタモードを世代別に変更し、以前のモードを返します。
* `"param"`：コレクタのパラメータの値を変更および/または取得します。このオプションの後には、1 つまたは 2 つの追加引数を続ける必要があります：変更または取得されるパラメータの名前（文字列）と、そのパラメータのオプションの新しい値（[0,100000] の範囲の整数）。最初の引数は次のいずれかの値である必要があります：
    * `"minormul"`：マイナー乗数。
    ...
    * `"stepsize"`：ステップサイズ。
    ...
    呼び出しは常にパラメータの以前の値を返します。呼び出しが新しい値を提供しない場合、値は変更されません。

    Lua はこれらの値を圧縮形式で保存するため、以前の値として返される値は、最後に設定された値と完全には一致しない場合があります。
