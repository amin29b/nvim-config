...
syn keyword	cTYPES	        u64 u32  u16 u8
syn keyword	cTYPES	        u64x u32x  u16x u8x
syn keyword	cTYPES	        s64 s32  s16 s8  
syn keyword	cTYPES	        s64x s32x  s16x s8x  
syn keyword	cTYPES	        r32 r64 b32
syn keyword	cTYPES	        r32x r64x b32x

syn keyword	cTYPES	        i64 i32  i16 i8  
syn keyword	cTYPES	        uint64 uint32 uint16 uint8 real32 real64 bool32
syn keyword	cTYPES	        int64 int32 int16 int8

syn keyword	cTYPES	        int64 int32 int16 int8
syn keyword	cTYPES		int long short char void
syn keyword	cTYPES		signed unsigned float double


syn keyword	cTYPES          int8_t  int16_t int32_t int64_t int8_t int16_t int32_t int64_t uint8_t uint16_t uint32_t uint64_t
syn keyword	cTYPES          uint8_t uint16_t uint32_t uint64_t float double float double  

syn keyword	cTodo               contained TODO  XXX
syn keyword	cNote               contained NOTE
syn keyword	cStudy              contained STUDY
syn keyword	cImportant          contained IMPORTANT warning
syn keyword	cFixMe              contained FIXME
syn keyword	cBug                contained BUG
syn match	cExtraWhiteSpace    "\s\+$"

""""syn match	cMarker              contained "\*x\*"

syn keyword	cASSERT          Assert 
call matchadd('cMarker', '\/\*x\*\/')

 ",cStudy,cImportant,cFixMe,cBug      
...
syn cluster	cCommentGroup	contains=cMarker,cTodo,cNote,cBadContinuation,cStudy,cImportant,cFixMe,cBug,cExtraWhiteSpace
...
if exists("c_comment_strings")
  ...
  "syn region  cCommentL2	start="//===" skip="\\$" end="$" keepend contains=@cCommentGroup,cComment2String,cCharacter,cNumbersCom,cSpaceError,cWrongComTail,@Spell
  syn match  cCommentL2	"^[ ]*//\s*[=]\+.*$" keepend contains=@cCommentGroup,cComment2String,cCharacter,cNumbersCom,cSpaceError,cWrongComTail,@Spell
  ...
  "syn region	cCommentL2	start="//===" skip="\\$" end="$" keepend contains=@cCommentGroup,cSpaceError,@Spell
  syn match	cCommentL2	"^[ ]*//\s*[=]\+.*$" keepend contains=@cCommentGroup,cSpaceError,@Spell
  if exists("c_no_comment_fold")
...
if !exists("c_no_c99") " ISO C99
  ...
  syn keyword cCONST2 true false Pi32
...
if !exists("c_no_c23")
  syn region	cPreCondit	start="^\s*\zs\%(%:\|#\)\s*\%(el\)\=\%(if\|ifdef\|ifndef\)\>" skip="\\$" end="$" keepend contains=cComment,cCommentL,cCommentL2,cCppString,cCharacter,cCppParen,cParenError,cNumbers,cCommentError,cSpaceError
else
  syn region	cPreCondit	start="^\s*\zs\%(%:\|#\)\s*\%(if\|ifdef\|ifndef\|elif\)\>"    skip="\\$" end="$" keepend contains=cComment,cCommentL,cCommentL2,cCppString,cCharacter,cCppParen,cParenError,cNumbers,cCommentError,cSpaceError
endif
...
if !exists("c_no_c23") && !s:in_cpp_family
  syn region	cInclude	start="^\s*\zs\%(%:\|#\)\s*embed\>" skip="\\$" end="$" keepend contains=cEmbed,cComment,cCommentL,cCommentL2,cCppString,cCharacter,cCppParen,cParenError,cNumbers,cCommentError,cSpaceError
...
" Optional embedded Autodoc parsing
if exists("c_autodoc")
  syn match cAutodocReal display contained "\%(//\|[/ \t\v]\*\|^\*\)\@2<=!.*" contains=@cAutodoc containedin=cComment,cCommentL,cCommentL2
...
if s:ft ==# 'c' || exists("cpp_no_cpp11")
  ...
  syn match	cMulti2		"?" contains=@cMulti
  syn match	cMulti2		":" contains=@cMulti
endif
...
"syn region  cCommentSEPERATOR	start="//" skip="\\$" end="$" keepend contains=@cCommentGroup,cComment2String,cCharacter,cNumbersCom,cSpaceError,cWrongComTail,@Spell
...
hi def link cCommentL2          CommentSEPERATOR
hi def link cStatement		BRANCH
hi def link cMulti2		BRANCH
hi def link cLabel		BRANCH
hi def link cUserLabel		BRANCH
hi def link cConditional	BRANCH
hi def link cTYPES	        TYPES
hi def link cRepeat		BRANCH
hi def link cNumber		CONST2
hi def link cCONST2		CONST2
hi def link cString		CONST2
hi def link cPreConditMatch	BRANCH
hi def link cPreCondit		BRANCH
"hi def link cType		BRANCH
hi def link cTodo		TODO
hi def link cNote		NOTE
hi def link cStudy	        STUDY
hi def link cImportant          IMPORTANT
hi def link cFixMe              FIXME    
hi def link cBug                BUG      
hi def link cASSERT             ASSERT
hi def link cWARNING            WARNING
hi def link cMarker             MARKER





"hi def link cBlock	CONST2
hi def link cExtraWhiteSpace	ExtraWhiteSpace
